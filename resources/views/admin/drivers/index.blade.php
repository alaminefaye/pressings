@extends('layouts.admin')

@section('title', 'Gestion des Livreurs')

@section('content')
<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h5 class="mb-0">Gestion des Livreurs</h5>
                <a href="{{ route('admin.drivers.create') }}" class="btn btn-primary">
                    <i class="bx bx-plus"></i> Nouveau Livreur
                </a>
            </div>
            
            <div class="card-body">
                <form method="GET" action="{{ route('admin.drivers.index') }}" class="row g-3 mb-4">
                    <div class="col-md-9">
                        <input type="text" name="search" class="form-control" placeholder="Rechercher..." value="{{ request('search') }}">
                    </div>
                    <div class="col-md-3">
                        <button type="submit" class="btn btn-primary me-2">
                            <i class="bx bx-search"></i> Rechercher
                        </button>
                        <a href="{{ route('admin.drivers.index') }}" class="btn btn-outline-secondary">
                            <i class="bx bx-reset"></i>
                        </a>
                    </div>
                </form>
            </div>

            <div class="table-responsive text-nowrap">
                <table class="table table-hover">
                    <thead>
                        <tr>
                            <th>Livreur</th>
                            <th>Téléphone</th>
                            <th>Email</th>
                            <th>Livraisons</th>
                            <th>Statut</th>
                            <th>Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @forelse($drivers as $driver)
                        <tr>
                            <td>
                                <div class="d-flex align-items-center">
                                    <div class="avatar avatar-sm me-3">
                                        <span class="avatar-initial rounded-circle bg-label-success">
                                            {{ strtoupper(substr($driver->first_name, 0, 1)) }}{{ strtoupper(substr($driver->last_name, 0, 1)) }}
                                        </span>
                                    </div>
                                    <div>
                                        <strong>{{ $driver->full_name }}</strong>
                                    </div>
                                </div>
                            </td>
                            <td>{{ $driver->phone }}</td>
                            <td>{{ $driver->email ?? '-' }}</td>
                            <td>
                                <span class="badge bg-label-success">{{ $driver->deliveries_count }} livraisons</span>
                            </td>
                            <td>
                                @if($driver->is_active)
                                    <span class="badge bg-label-success">Actif</span>
                                @else
                                    <span class="badge bg-label-secondary">Inactif</span>
                                @endif
                            </td>
                            <td>
                                <a href="{{ route('admin.drivers.edit', $driver->id) }}" class="btn btn-sm btn-outline-primary">
                                    <i class="bx bx-edit"></i>
                                </a>
                                <form method="POST" action="{{ route('admin.drivers.destroy', $driver->id) }}" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-sm btn-outline-danger" onclick="return confirm('Êtes-vous sûr?')">
                                        <i class="bx bx-trash"></i>
                                    </button>
                                </form>
                            </td>
                        </tr>
                        @empty
                        <tr>
                            <td colspan="6" class="text-center">Aucun livreur trouvé</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            </div>

            <div class="card-footer">
                {{ $drivers->links() }}
            </div>
        </div>
    </div>
</div>
@endsection


